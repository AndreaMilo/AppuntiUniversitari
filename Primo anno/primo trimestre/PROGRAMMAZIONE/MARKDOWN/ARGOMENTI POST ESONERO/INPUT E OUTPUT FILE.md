...
## FILE BINARI
I file binari sono denominati anche **file ad accesso casuale**, poiché il puntatore non scorre sequenzialmente i contenuti, di fatto l'accesso avviene puntando ad una specifica locazione di memoria.
I file binari garantiscono una maggiore **flessibilità**, i contenuti possono essere aggiornati senza **sovrascrivere** i dati memorizzati precedentemente.
### FILE BINARI AD ACCESSO CASUALE
I dati in un file ad accesso casuale **non sono formattati**, sono memorizzati come *raw bytes*, tutti i tipi di dati utilizzano la stessa quantità di memoria, come tutti i record hanno la stessa lunghezza fissa, in più il formato dei dati non sono **Human readable** ovvero usano un linguaggio difficile e non facilmente comprensibile alla lettura di un essere umano.
### LETTURA E SCRITTURA IN BLOCCHI DI BYTE
Esistono varie funzioni che ci permettono di scrivere su blocchi di byte o anche di dati testuali; queste funzioni trovano utilizzo specialmente nella scrittura di un intera struct su un file.
- $int fread$*(void \*punt,dimBlocco,numBlocchi, FILE \*fp)*: Legge dal file *fp* un numero di byte pari alla **dimensione dei blocchi \* numero dei blocchi** e li memorizza nell'area di memoria puntata dal **puntatore**. Restituendo in fine il numero di blocchi letti. 
- $int fwrite$*(void \*punt, dimBlocco, numBlocchi, FILE \*fp)*: Esegue la stessa operazione precedente ma in **scrittura**.

## FSEEK
La funzione $FSEEK$ ci permette di accedere a degli specifici byte del file, come se quest'ultimo fosse un array di blocchi di byte.
La funzione:*int fseek(FILE \*fp, long offset, int refpoint)*, imposta la posizione corrente ad un valore pari ad uno spostamento del valore dell'**offset**, il quale viene calcolato in base a dei punti di partenza: 
- All'inizio del file se refpoint vale SEEK_SET che è un valore costante impostato dalla libreria *stdio.h*
- L'attuale posizione corrente, la quale è un altra costante della medesima libreria
- La fine del file se refpoint vale SEEK_END
Il valore restituito dall'offset è 0 se l'operazione è avvenuta con successo altrimenti ha un valore differente.

### REWIND
```
void rewind(FILE *fp)
```
La funzione **rewind** è utile per *riavvolgere* la posizione corrente del file senza doverlo necessariamente chiudere e riaprirlo. Per esempio se volessimo tornare a contare il file dall'inizio possiamo avvalerci di questa funzione.
Tra la chiamata della funzione $rewind$ e la chiamata alla funzione di *fseek(fp,0,SEEK_SET)*, non vi è alcuna differenza, entrambe svolgono la stessa funzionalità ma è buona pressi utilizzare rewind poiché rende il codice più leggero e aumenta la sua **leggibilità**, dato che non dovremmo specificare tutti i parametri come nella funzione $fseek$.
èèèè
